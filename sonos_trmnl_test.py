...
from display_image import generate_image, generate_image_from_file, show_image
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        if prev_title != title or prev_artist != artist:
            ...
            # just for testing purposes but can go away later -- was using this machine as if it was a TRMNL device
            try:
                response = requests.get(terminus_api_url+"/api/display", headers=headers, verify=False)
                print(f"API Response: {response.status_code}")
                if response.status_code != 200:
                    print(f"Error: {response.text}")
            except Exception as e:
            ...
            r = response.json()
            print(r)
            img = generate_image(r["image_url"], w=800, h=480)
            show_image(img)
            # End of development code

            hti.screenshot(html_str=html_content, save_as='sonos_display.png', size=(800, 600))
            #img2 = generate_image_from_file('sonos_display.png', w=800, h=480)
            img2 = generate_image_from_file('sonos_display.png')
            show_image(img2)
           # f = BytesIO()
           # img.save(f)
           # img.close()
